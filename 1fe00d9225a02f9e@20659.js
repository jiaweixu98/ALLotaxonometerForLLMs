...
md`# ALLotaxonometer for all`
)}

function _2(md){return(
md`_Warning: don't use the Safari browser. For some reasons the canvas fails to rotate._

Upload your own \`.json\` file where each key is a separate system (see [repo](https://github.com/jstonge/allotaxonometer) for an example):`
)}
...
md`Or two tabular files (if you change mind during the session and you want to use CSVs, refresh the page to get rid of the \`.json\` file. See [repo](https://github.com/jstonge/allotaxonometer) for the format that the allotax expects.)`
)}
...
md`Or use an example from the original paper (note that when file size gets larger there is a time delay. Be patient):`
)}

function _sel_data(Inputs){return(
Inputs.radio(["Girl babynames", "Boy babynames", "Twitter", "Species Abundance"], {label: "", value: "Girl babynames"})
)}
